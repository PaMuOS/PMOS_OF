...
//----------------------------------- KINECT -------------------------------------------
    ...
    grayThresFar.allocate(kinect.width, kinect.height);
...
void AppCore::update() {
    ...
    if(kinect.isFrameNew()){
        ...
        grayThresNear.threshold(180);
        contourFinder.findContours(grayThresNear, 200, (kinect.width*kinect.height)/3, maxInput, false);
    }
...
void AppCore::draw() {
    ...
    grayThresNear.draw(0, 0, screenWidth, screenHeight);
    contourFinder.draw(0, 0, screenWidth, screenHeight);
    //kinect.drawDepth(0, 0, screenWidth, screenHeight);
    
    for(int i = 0; i < contourFinder.nBlobs; i++) {
        blobs[i] = contourFinder.blobs.at(i);
        blobCenterX[i] = blobs[i].centroid.x;
        blobCenterXmap[i] = ofMap(blobCenterX[i], 0, kinect.width, 0, screenWidth);
        blobCenterY[i] = blobs[i].centroid.y;
        blobCenterYmap[i] = ofMap(blobCenterY[i], 0, kinect.height, 0, screenHeight);
        
        // draw center of blobs
        ofSetColor(255, 255, 0);
        ofFill();
        ofCircle(blobCenterXmap[i], blobCenterYmap[i], 2);
        ofSetColor(0);
        
        kinectInput(blobCenterXmap[i], blobCenterYmap[i]);
    }
...
void AppCore::kinectInput(float _x, float _y){
    ...
    for(int i=0; i<TUBE_NUM; i++){
        float distance = ofDist(allPipes[i]->x, allPipes[i]->y, inputX, inputY);
